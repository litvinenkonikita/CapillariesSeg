...
BATCH_SIZE = 8 # 4 # 8 # 2
LR1 = 1e-3 # 1e-4
NUM_OF_EPOCHS = 10 # 200 # 175 # 160 # 150 # 300
...
BCE_WEIGHT = 0.5 # 0.05 # 0.175? # 0.66667 # 0.95! # 0.825! # 0.01? # 0.99! # 0.6? # 0.9! # 0.75! # 0.3 # 0.35 # 0.1 # 0.25!
...
class CapillariesSegDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        mask[np.logical_not(np.isin(mask, [2, 3]))] = 0
        mask[np.isin(mask, [2, 3])] = 1
    ...
    def mask_multi_hot_encoding(self, mask_tensor):
        ...
        return np.stack((nail_mask, capillary_mask), axis=2).astype(np.float32)
...
"""
def calc_loss(pred, target, metrics, bce_weight=0.5): # !!! LOSS = binary_cross_entropy_with_logits !!!
...
"""
def get_best_threshold(preds, masks, init_thresh=0.01, stop=1.0, step=0.01): # init_thresh=0.01, stop=1.0, step=0.01):
    best_thresh = init_thresh
    metric = BinaryJaccardIndex(threshold=init_thresh)
    preds_tensor = torch.from_numpy(preds)
    masks_tensor = torch.from_numpy(masks)
    best_metric = metric(preds_tensor, masks_tensor)
    for thresh in np.arange(init_thresh, stop, step):
        metric = BinaryJaccardIndex(threshold=thresh)
        thresh_metric = metric(preds_tensor, masks_tensor)
...
def get_best_threshold(preds, masks, init_thresh=0.01, stop=1.0, step=0.01): # init_thresh=0.01, stop=1.0, step=0.01):
    ...
    preds_tensor = preds.copy()
    masks_tensor = masks.copy()

    preds_tensor[preds_tensor >= best_thresh] = 1
    preds_tensor[preds_tensor < best_thresh] = 0

    preds_tensor = torch.from_numpy(preds_tensor.astype(np.uint8))
    masks_tensor = torch.from_numpy(masks_tensor.astype(np.uint8))
    ...
    for thresh in np.arange(init_thresh+step, stop, step):
        ...
        preds_tensor[preds_tensor >= thresh] = 1
        preds_tensor[preds_tensor < thresh] = 0
...
def save_predictions(loader, model, preds_path, mIoU_filename='mIoU'):
    ...
    with torch.no_grad():
        for frames, masks in loader:
            ...
            masks_rgb = [reverse_transform_masks_preds(x) for x in masks]
            ...
            for mask_ in masks:
                capillaries_masks = np.append(capillaries_masks, mask_, axis=0)

            for pred_ in preds:
                capillaries_preds = np.append(capillaries_preds, pred_, axis=0)

        thresh, metric = get_best_threshold(capillaries_preds, capillaries_masks)
        iou = f'\nthresh: {thresh}\nmetric: {metric}'
        print(iou)
        ...
        capillaries_preds[capillaries_preds < thresh] = 0
        capillaries_preds[capillaries_preds >= thresh] = 255
...
if __name__ == '__main__':
    ...
    train_loader = DataLoader(train_aug_dataset, batch_size=BATCH_SIZE, shuffle=True, num_workers=n_cpu//3)
    ...
    test_dataloader = DataLoader(test_dataset, batch_size=BATCH_SIZE, shuffle=True, num_workers=n_cpu//5)
